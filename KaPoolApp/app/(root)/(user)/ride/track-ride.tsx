...
const TrackRide = () => {
  ...
  const filteredDriver = drivers.filter(
    (driver) => driver.driver_id._id === ride.driver_id
  );
  ...
  const handleError = (error) => {
    if (error.response) {
      ...
      Alert.alert(
        "Error",
        error.response.data.message || "Failed to end the ride."
      );
  ...
  const handleCancelRide = async () => {
    try {
      ...
      Alert.alert(
        "Ride Cancelled",
        "You have successfully cancelled the ride."
      );
      ...
      <>
        <Text className="text-2xl font-semibold mb-2">
          {rideStatus === "arrived"
            ...
            : `Arriving in ${formatTime(
                Number(filteredDriver[0]?.time?.toFixed(0)) || 0
              )}`}
        </Text>
        ...
        <View className="flex flex-col w-full items-start justify-center">
          <View className="flex flex-row items-center justify-between w-full border-b border-white py-2">
            ...
            <Text className="text-md font-normal text-[#0CC25F]">
              ₦{Number(ride.fare_price).toFixed(0)}
            </Text>
          </View>
          ...
          <View className="flex flex-row items-center justify-between w-full border-b border-white py-2">
            ...
            <Text className="text-md font-normal">
              {formatTime(Number(ride.ride_time?.toFixed(0))!)}
            </Text>
          </View>
          ...
          <View className="flex flex-row items-center justify-between w-full py-2">
            ...
            <Text className="text-md font-normal">
              {filteredDriver[0]?.vehicle_seats} seats
            </Text>
          </View>
        </View>
        ...
        <View className="flex flex-col w-full items-start justify-center">
          ...
          <View className="flex flex-row items-center justify-start border-b border-general-700 w-full py-3">
            ...
            <Text className="text-md font-normal ml-2">
              {destinationAddress}
            </Text>
          </View>
        </View>
        ...
        >
          <View className="bg-white text-center items-center rounded-lg p-5">
            ...
            <Text className="my-3 text-lg text-gray-600 text-center">
              You have to wait longer if you cancel. Rebooking may not get you
