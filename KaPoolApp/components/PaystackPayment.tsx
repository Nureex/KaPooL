...
import { API_URL } from "@/lib/utils"; // Adjust the path as necessary
import Constants from "expo-constants"; // Import Expo constants

const paystackPublicKey = Constants.expoConfig?.extra?.paystackPublicKey; // Access API key
...
const PaystackPayment = ({ amount, email, reference, onSuccess, ride }) => {
  ...
  const handlePaystackSuccess = async (responseData) => {
    try {
      ...
      if (
        ...
        verifyResponse.data.payment.status === "paid"
      ) {
  ...
  return (
    <View style={{ flex: 1 }}>
      <Paystack
        showPayButton={false}
        paystackKey={paystackPublicKey} // Replace with your Paystack public key
        amount={amount}
        billingEmail={email}
